...
(function(exports2) {
  ...
  exports2.__defineGetter__("Types", () => requireTypes());
...
async function query(sql, params = []) {
  ...
  try {
    const [rows, fields2] = await conn.execute(sql, params);
...
electron.app.whenReady().then(async () => {
  try {
    ...
    if (!connected) {
      electron.dialog.showErrorBox("数据库连接失败", "无法连接到MySQL数据库，请确保MySQL服务已启动并配置正确");
    }
    setupIpcHandlers();
    createWindow();
  } catch (error) {
