...
function keyFromFields(type2, fields2, options, config) {
  const res = [
    type2,
...
function getParser(type2, fields2, options, config, compiler) {
  const key2 = keyFromFields(type2, fields2, options, config);
      ...
      if (typeof BigInt === "function") {
        ...
        Long2.fromValue = function fromValueWithBigInt(value, unsigned) {
          if (typeof value === "bigint")
            return Long2.fromBigInt(value, unsigned);
...
const CharsetToEncoding$8 = charset_encodings;
...
function toParameter(value, encoding, timezone) {
  ...
  return { value, type: type2, length: length2, writer };
}
...
function requireCaching_sha2_password() {
  if (hasRequiredCaching_sha2_password)
...
function requireMysql_native_password() {
  if (hasRequiredMysql_native_password)
...
function requireMysql_clear_password() {
  if (hasRequiredMysql_clear_password)
...
function typeMatch(type2, list, Types2) {
  if (Array.isArray(list)) {
    return list.some((t) => type2 === Types2[t]);
  }
  return !!list;
}
...
function readCodeFor$1(type2, charset, encodingExpr, config, options) {
  ...
  switch (type2) {
    ...
    case Types$3.DATE:
      if (helpers$3.typeMatch(type2, dateStrings, Types$3)) {
        return 'packet.readLengthCodedString("ascii")';
      }
...
function readField({ packet: packet2, type: type2, charset, encoding, config, options }) {
  ...
  switch (type2) {
    ...
    case Types$2.DATE:
      if (helpers$2.typeMatch(type2, dateStrings, Types$2)) {
        return packet2.readLengthCodedString("ascii");
      }
...
class PromisePool extends EventEmitter$1 {
  ...
  releaseConnection(connection2) {
    if (connection2 instanceof PromisePoolConnection$1)
      connection2.release();
  }
  query(sql, args) {
    const corePool = this.pool;
...
(function(exports2) {
  ...
  exports2.escape = SqlString2.escape;
  exports2.escapeId = SqlString2.escapeId;
  exports2.format = SqlString2.format;
  exports2.raw = SqlString2.raw;
  exports2.PromisePool = PromisePool2;
  exports2.PromiseConnection = PromiseConnection3;
  exports2.PromisePoolConnection = PromisePoolConnection3;
  exports2.__defineGetter__("Types", () => typesExports);
...
electron.app.whenReady().then(async () => {
  try {
    ...
    if (!connected) {
      electron.dialog.showErrorBox("数据库连接失败", "请检查数据库配置和确保MySQL服务已启动");
    }
    setupIpcHandlers();
    createWindow();
  } catch (error) {
